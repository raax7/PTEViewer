...
struct IMGUI_API ImGuiTypingSelectRequest
{
    ...
    int                     SearchBufferLen;
    const char*             m_SearchBuffer;       // Search buffer contents (use full string. unless SingleCharMode is set, in which case use SingleCharSize).
...
struct IMGUI_API ImGuiTypingSelectState
{
    ...
    char            m_SearchBuffer[64];           // Search buffer: no need to make dynamic as this search is very transient.
    ImGuiID         FocusScope;
    ...
    void            Clear()  { m_SearchBuffer[0] = 0; SingleCharModeLock = false; } // We preserve remaining data for easier debugging
};
