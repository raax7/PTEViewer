...
ImGuiTypingSelectRequest* ImGui::GetTypingSelectRequest(ImGuiTypingSelectFlags flags)
{
    ...
    // Append to buffer
    const int buffer_max_len = IM_ARRAYSIZE(data->m_SearchBuffer) - 1;
    int buffer_len = (int)strlen(data->m_SearchBuffer);
    ...
    for (ImWchar w : g.IO.InputQueueCharacters)
    {
        ...
        if (data->SingleCharModeLock && w_len == out_request->SingleCharSize && memcmp(w_buf, data->m_SearchBuffer, w_len) == 0)
        {
        ...
        memcpy(data->m_SearchBuffer + buffer_len, w_buf, w_len + 1); // Append
        buffer_len += w_len;
        select_request = true;
    }
    ...
    {
        char* p = (char*)(void*)ImTextFindPreviousUtf8Codepoint(data->m_SearchBuffer, data->m_SearchBuffer + buffer_len);
        *p = 0;
        buffer_len = (int)(p - data->m_SearchBuffer);
    }
    ...
    out_request->m_SearchBuffer = data->m_SearchBuffer;
    ...
    {
        const char* buf_begin = out_request->m_SearchBuffer;
        const char* buf_end = out_request->m_SearchBuffer + out_request->SearchBufferLen;
...
int ImGui::TypingSelectFindNextSingleCharMatch(ImGuiTypingSelectRequest* req, int items_count, const char* (*get_item_name_func)(void*, int), void* user_data, int nav_item_idx)
{
    ...
    for (int idx = 0; idx < items_count; idx++)
    {
        ...
        if (ImStrimatchlen(req->m_SearchBuffer, req->m_SearchBuffer + req->SingleCharSize, item_name) < req->SingleCharSize)
            continue;
...
int ImGui::TypingSelectFindBestLeadingMatch(ImGuiTypingSelectRequest* req, int items_count, const char* (*get_item_name_func)(void*, int), void* user_data)
{
    ...
    {
        const char* item_name = get_item_name_func(user_data, idx);
        const int match_len = ImStrimatchlen(req->m_SearchBuffer, req->m_SearchBuffer + req->SearchBufferLen, item_name);
...
{
#ifndef IMGUI_DISABLE_DEBUG_TOOLS
    Text("SearchBuffer = \"%s\"", data->m_SearchBuffer);
